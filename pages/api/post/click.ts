...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const country = Array.isArray(req.query.country) ? req.query.country[0] : (req.query.country ?? 'unknown');
  const userAgent = Array.isArray(req.query.useragent) ? req.query.useragent[0] : (req.query.useragent ?? 'unknown');
  const ip = Array.isArray(req.query.ip) ? req.query.ip[0] : (req.query.ip ?? 'unknown');
  const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent);
  const userId = sub;
  ...
  const rawEncoded = Buffer.from(`${sub},${country},${ip},${gadget},${networkId}`).toString('base64');
